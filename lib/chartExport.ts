...
import { CORES } from '@/lib/visual/brand';

const CHART_TITLE_COLOR = '#059669';
const CHART_SUBTITLE_COLOR = '#64748b';
...
export async function downloadChartCardPng(
  ...
  if (subtitleEl?.textContent) {
    ctx.fillStyle = CHART_SUBTITLE_COLOR;
...
<head>
  ...
  <style>
    ...
    .chart-title { font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.18em; color: ${CHART_TITLE_COLOR}; }
    .chart-subtitle { margin-top: 4px; font-size: 14px; color: ${CHART_SUBTITLE_COLOR}; }
...
/** Cor da linha temporal nos gráficos exportados (referência). */
export const LINE_CHART_STROKE = CORES.primaryDark;
