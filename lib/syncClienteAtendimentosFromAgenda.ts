...
type ClientesDriveStore = Awaited<ReturnType<typeof loadClientesStore>>;
...
/** Data/hora da consulta no fuso BR (bate com finalização na agenda). */
...
function tipoAtendimentoFromConsulta(tipo: string | null): 'consulta' | 'retorno' {
  return tipo === 'retorno' ? 'retorno' : 'consulta';
}
...
async function fetchRealizadasForOwner(
  ...
  let q = supabaseAdmin
    ...
    .select(
      'id, cliente_drive_id, inicio, paciente, servico, medico, observacoes, tipo_consulta',
    )
...
/**
 * Cria atendimentos no Drive para consultas `realizado` na agenda que não
...
export async function syncRealizadasAgendaToClienteDrive(
  ...
  if (opts?.clienteId) {
    const cliente = findCliente(store, opts.clienteId);
    if (!cliente) {
  ...
  for (const c of consultas) {
    ...
    const obsServico =
      c.servico && c.servico !== 'Consulta' ? `Serviço: ${c.servico}` : null;
    ...
    finalizarAtendimentoNoCliente(cliente, {
      ...
      medico: c.medico,
      parcelas: 1,
      tipo: tipoAtendimentoFromConsulta(c.tipo_consulta),
      observacoes,
    });
    created += 1;
  }
