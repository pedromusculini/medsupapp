...
/** CSV consolidado dos três gráficos. */
export function gerarCsvGraficos(data: {
  ...
  linhas.push('');
  linhas.push('=== REPASSE POR MÉDICO ===');
  linhas.push('Médico;Parte médico (R$)');
  for (const m of data.porMedico) {
    linhas.push(`${m.nome};${m.valor.toFixed(2)}`);
  }
  ...
  linhas.push(`${new Date().toLocaleString('pt-BR')};${PRODUCT_NAME}`);
