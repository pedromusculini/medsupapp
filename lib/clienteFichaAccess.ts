...
> {
  const { data: link, error } = await supabaseAdmin
    ...
    .select('owner_email, ativo, expires_at, cliente_drive_id')
    .eq('token', token)
    .maybeSingle();
  ...
  if (!ownerEmail) {
    return { ok: false, status: 400, error: 'Link sem clínica associada' };
  }
...
/** Titular da clínica ou médico de equipe com agenda Google conectada ao mesmo owner_email. */
export async function canAccessClienteFichaProfissional(params: {
  ...
  return {
    allowed: false,
    reason:
      'Você não tem permissão para ver esta ficha. Entre com o e-mail cadastrado na equipe e confirme que a Agenda Google está conectada.',
  };
}
