...
export function supabaseErrorMessage(error: unknown, fallback: string): string {
  if (isSupabaseNetworkError(error)) {
    return 'Não foi possível conectar ao banco de dados. Verifique SUPABASE_URL e a rede.';
  }
  if (isSupabaseMissingColumnError(error)) {
    return 'Colunas de pagamento ainda não existem no banco. Execute: npm run db:config-pagamento && npm run db:financeiro-profissional';
  }
