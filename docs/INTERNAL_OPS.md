...
```
middleware.ts          → 404 se path /internal ou /api/internal e e-mail ∉ ADMIN_EMAILS
...
app/internal/               → UI (sem link no Header)
app/api/internal/           → overview, tenants, tenants/[email], notes
```
...
|--------|------|-----------|
| GET | `/api/internal/overview` | KPIs globais |
| GET | `/api/internal/tenants?q=&filter=&limit=&offset=` | Lista paginada (`filter`: ver tabela acima) |
| GET | `/api/internal/tenants/[email]` | Ficha + `notes` + `audit_log` + log `view_tenant` |
| GET/POST | `/api/internal/tenants/[email]/notes` | Listar / criar nota interna |
| POST | `/api/internal/tenants/[email]/reset-access` | Reset de acesso (`mode`: `reverify` ou `remove`) |
...
- `robots.txt`: `Disallow` `/internal` e `/api/internal`.
