    ...
    if (allConnected) {
      ...
      for (const id of connectedIds) {
        ...
        try {
          ...
          console.warn(`[google-calendar/GET] médico ${id}:`, err);
        }
      }
      ...
      if (!allItems.length && !titularAuth && !connectedIds.length) {
        return NextResponse.json(
          {
            error:
              'Nenhuma agenda conectada. Conecte sua agenda ou peça aos médicos que autorizem.',
          },
          { status: 403 },
        );
      }
      ...
      return NextResponse.json(
        {
          error: profissionalId
            ? 'Agenda deste médico não está conectada. Envie o convite pelo WhatsApp.'
  ...
  try {
    ...
    if (!authCtx) {
      return NextResponse.json(
        {
          error: profissionalId
            ? 'Agenda deste médico não está conectada.'
