...
type Props = {
  ...
  redirectPath?: string;
  className?: string;
  /** Mensagem extra contextual (ex.: Pacientes, Agenda) */
  context?: 'clientes' | 'agenda' | 'geral';
};

const CONTEXT_COPY: Record<NonNullable<Props['context']>, string> = {
  geral:
    'O MedSupAPP depende do Google Drive e do Google Calendar. Sem conexão ativa, cadastros e agenda não são salvos.',
  clientes:
    'Pacientes e prontuários ficam no seu Google Drive. Reconecte o Google para salvar e listar cadastros.',
  agenda:
    'A agenda usa o Google Calendar. Reconecte o Google para criar e sincronizar consultas.',
};
        ...
        <div className="flex flex-1 gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <span
                className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${
                  data.contacts ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'
                }`}
              >
