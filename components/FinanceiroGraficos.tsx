...
function ChartActionButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-emerald-700 transition hover:border-emerald-400 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-40"
    >
...
export default function FinanceiroGraficos({
  ...
  const hasAnyData =
    porForma.length > 0 || porMedico.length > 0 || porPeriodo.length > 0;

  const handleExportAllCsv = () => {
    const csv = gerarCsvGraficos({ porForma, porMedico, porPeriodo });
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <ChartCard
          title="Repasse por médico"
          subtitle="Valor do repasse ao médico (parte médico)."
          slug="repasse-medico"
          periodSuffix={periodSuffix}
          isEmpty={porMedico.length === 0}
          emptyMessage="Nenhuma entrada com médico no período."
          csvRows={medicoCsvRows(porMedico)}
        >
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              key={medicoChartKey}
            ...
            >
              ...
              />
              <Bar dataKey="valor" name="Parte médico" radius={[0, 6, 6, 0]}>
                {porMedico.map((_, i) => (
                  <Cell
                    key={porMedico[i].nome}
                    fill={CHART_COLORS[i % CHART_COLORS.length]}
                  />
                ))}
              </Bar>
...
function medicoCsvRows(data: MedicoBar[]): string[][] {
  return [
    ["Médico", "Parte médico (R$)"],
    ...data.map((m) => [m.nome, m.valor.toFixed(2)]),
  ];
}
